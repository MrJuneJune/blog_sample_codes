 ...
 * exists, return an empty array. The average value of a set of k numbers is the
 * sum of the numbers divided by k. Note that mean is an integer, so the sum of
 * the n + m rolls should be divisible by n + m.
 *
 */
#include <iostream>
class Solution {
 public:
    vector<int> missingRolls(vector<int>& rolls, int mean, int n) {
        int totalVal = mean * (rolls.size() + n);
        std::cout << totalVal;
    }
};
